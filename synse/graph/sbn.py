...
class SBNSpec:
    ...
    NEW_BOX_INDICATORS = '|'.join([
        'ALTERNATION', 'ATTRIBUTION', 'CONDITION', 'CONSEQUENCE',
        'CONTINUATION', 'CONTRAST', 'EXPLANATION', 'NECESSITY', 'NEGATION',
        'POSSIBILITY', 'PRECONDITION', 'RESULT', 'SOURCE'
    ])
            ...
            while len(tokens) > 0:
                ...
                if tok_count == 0 and (
                    ...
                    box_edge = self.create_edge(
                        self._active_box_id(), self._active_sense_id(),
                    ...
                    box_edge = self.create_edge(
                        current_box_id, self._active_box_id(),
                        SBN_EDGE_TYPE.BOX_CONNECT, token
                    )
                    ...
                    if SBNSpec.MIN_SENSE_IDX <= target_idx <= max_wn_idx:
                        role_edge = self.create_edge(
                            self._active_sense_id(), to_id,
                            SBN_EDGE_TYPE.ROLE, target
                        )
                        ...
                        role_edge = self.create_edge(
                            self._active_sense_id(), const_node[0],
                            SBN_EDGE_TYPE.ROLE, target
                        )
                        box_edge = self.create_edge(
                            self._active_box_id(), const_node[0],
                            SBN_EDGE_TYPE.BOX_CONNECT
                        )
                    ...
                    role_edge = self.create_edge(
                        self._active_sense_id(), name_node[0],
                        SBN_EDGE_TYPE.ROLE, target
                    )
                    box_edge = self.create_edge(
                        self._active_box_id(), name_node[0],
                        SBN_EDGE_TYPE.BOX_CONNECT
                    )
                    ...
                    name_node = self.create_node(
                        SBN_NODE_TYPE.CONSTANT, constant_match.group(0),
                        {'comment': comment}
                    )
                    role_edge = self.create_edge(
                        self._active_sense_id(), name_node[0],
